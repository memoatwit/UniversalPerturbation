...
def generate_fgsm_perturbation(images, target_model, input_label, epsilon, clip_value=1.0, targeted=False):
    ...
    for idimage, image in enumerate(images):
        ...
        signed_grad = -tf.sign(gradient) if targeted else tf.sign(gradient)#this is just a +/-1 matrix
        ...
        perturbation[idimage] = perturbation[idimage] + (epsilon * signed_grad)
        perturbation = np.clip(perturbation, -clip_value, clip_value)
...
####
def _single_iterative_perturbation(image, target_model, input_label, epsilon=1., num_iterations=100, regularization=1., clip_value=1.0, targeted=False):
    ...
    reshaped_input = tf.one_hot(input_label, 1_000)
    reshaped_input = tf.reshape(reshaped_input, (1, 1_000))
    ...
    for it in range(num_iterations):
        with tf.GradientTape() as tape:
            ...
            loss = tf.keras.losses.categorical_crossentropy(reshaped_input, predictions)
            ...
            signed_grad = -tf.sign(grads[0]) if targeted else tf.sign(grads[0])
            signed_grad = tf.stop_gradient(signed_grad)
...
####
def generate_iterative_perturbation(images, target_model, input_label, num_iterations=100, regularization=1., clip_value=1.0, epsilon=1.0, targeted=False):
    
    perturbations = np.zeros_like(images)

    for idimage, image in enumerate(images): #for each image
        print(f"*** Attacking image: {idimage}")
        perturbations[idimage] = _single_iterative_perturbation(image, target_model, input_label, epsilon, num_iterations=num_iterations, regularization=regularization, clip_value=clip_value, targeted = targeted)
...
def generate_universal_perturbation(images, target_model, input_label, epsilon=1., num_iterations=1, regularization=1., clip_value=1.0, targeted = False):
    ...
    target_label = tf.one_hot(input_label, 1000)
    target_label = [target_label, target_label, target_label, target_label, target_label]
    target_label = tf.reshape(target_label, (5,1000))
