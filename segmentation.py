...
def predict(image):
    with torch.no_grad():
        image = transform(to_pil_image(image)).unsqueeze(0).to("cuda:0")
...
def generate_segmented_noise(indx, image_org, seg_images, seg_image_masks, seg_map, learning_rate= 0.2, it=70):
    """
        This noise generator uses targetted Universal Pertubation methods instead
    Args:
        indx (_type_): _description_
        image_org (_type_): _description_
        seg_images (_type_): _description_
        seg_image_masks (_type_): _description_
        seg_map (_type_): _description_
        learning_rate (float, optional): _description_. Defaults to 0.1.
        it (int, optional): _description_. Defaults to 70.
    """

    print("Original Image Prediction : ")
    orj_res, orj_im = predict(image_org)
    if orj_res < 0.05:
        print(f"Confidence:{orj_res} is below threashold for input image, skipping...")
        to_pil_image(image_org).save(save_folder + "/" + data_list[indx])
        return
    ...
    pertubation = torch.from_numpy(pinit * seg_image_mask[None, None, :, :]).to('cuda:0')

    for step in range(it):
        pertubation.requires_grad = True

        image = image + pertubation

        outputs, _ = model(image, text)
        probs = outputs.softmax(dim=-1).detach().cpu().numpy()

        loss = criterion(outputs, torch.tensor([target_label], device="cuda:0"))
        model.zero_grad()
        loss.backward()
        ...
        if probs[0,true_label] == 0.0 and loss.item() < 2 and probs[0,target_label] == 1.0:
            break
        
        signed_grad = -pertubation.grad.sign() # -1 because we want to minimize loss for targetted attack

        with torch.no_grad():
            # image -= learning_rate * signed_grad
            # noise += learning_rate * signed_grad
            pertubation.add(signed_grad * learning_rate)
        
        pertubation = torch.from_numpy(pertubation.clone().detach().cpu().numpy() * seg_image_mask[None, None, :, :]).to("cuda:0")
        pertubation = clip_values(pertubation)
    ...
    resize_transform = transforms.Resize((image_org.shape[1], image_org.shape[2]))
    ...
    merged_img = merge(image_org, resized_image, best_mask)
...
def merge(image_org, resized_image, seg_map):

    masked_image_tensor = resized_image * seg_map["segmentation"][None, :, :]

    inverse_image_org = image_org * np.logical_not(seg_map["segmentation"][None, :, :])

    inverse_image_org += masked_image_tensor

    return inverse_image_org
...
def run(image_list):
    ...
    for i, img in tqdm(enumerate(image_list), leave=False):
        ...
        generate_segmented_noise(i, img, seg_images, seg_image_masks, seg_map)
...
if __name__ == '__main__':
    ...
    imagenet_classes = ["door", "window", "stair", "wall", "roof",
            "tree", "clock", "hat", "lemon", "baseball", "house",
            "traffic light", "mailbox", "tree",
            "fence", "parking meter", "sidewalk", "road",
            "pool", "playground", "bridge", 
            "handrail", "patio", "column", "downspout", "doorknob", "siding"]
    ...
    model, transform = clip.load(args.clip_model, device="cuda:0") 
    ...
    text = clip.tokenize(imagenet_classes).to("cuda:0")
